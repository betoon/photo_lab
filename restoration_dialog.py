...
class RestorationStudioDialog(QDialog):
    def __init__(self,parent,image,source_path=""):
        ...
        root=QHBoxLayout(self);panel=QScrollArea();panel.setWidgetResizable(True);panel.setMaximumWidth(430);body=QWidget();pv=QVBoxLayout(body);panel.setWidget(body);root.addWidget(panel);self.tabs=QTabWidget();self.tabs.setAccessibleName("Restoration workspaces");pv.addWidget(self.tabs);self._restore_tab();self._ai_tab();self.status=QLabel("All restoration is previewed non-destructively. Apply creates a new file.");self.status.setWordWrap(True);pv.addWidget(self.status);buttons=QDialogButtonBox(QDialogButtonBox.StandardButton.Cancel|QDialogButtonBox.StandardButton.Apply);buttons.rejected.connect(self.reject);buttons.accepted.connect(self._accept);pv.addWidget(buttons)
